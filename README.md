...
Copia l’output nel file evidenze.
---
## Troubleshooting - Registrazione del Provider Microsoft.Insights

Se durante l'esecuzione dello Step 8 il comando restituisce l'errore:
`Please register the subscription '...' with Microsoft.Insights`

Significa che il Resource Provider per il monitoraggio non è attivo sulla tua sottoscrizione. Risolvi seguendo questi passaggi:

### 1. Registra il provider
Esegui il comando per avviare la registrazione del servizio:

```bash
az provider register --namespace Microsoft.Insights
```
2. Verifica lo stato della registrazione
Il processo può richiedere 1-2 minuti. Esegui questo comando per controllare lo stato:

Bash
az provider show -n Microsoft.Insights --query registrationState
Nota: Procedi allo Step 9 solo quando il risultato è "Registered". Se visualizzi "Registering", attendi brevemente e ripeti il controllo. Una volta registrato, riesegui il comando dello Step 8.
