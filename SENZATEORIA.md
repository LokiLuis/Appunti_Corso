...
Questo laboratorio si colloca dopo il **LAB12** (Alerting) e prima del **LAB13** (Workbooks / Dashboard).

```text
LAB11    Log Analytics + KQL
LAB12    Alerting (Alert Rule + Action Group)
  ↓
...
- centralizzato i log in Log Analytics (LAB11)
- scritto query KQL esplorative (LAB11)
- creato un alert sull'error_rate (LAB12)

In questo laboratorio farai due cose nuove:

1. userai KQL per fare **analisi di performance** più avanzate sui dati già presenti
2. modificherai l'applicazione, simulerai un **incidente reale** e farai troubleshooting end-to-end

---

## Durata indicativa

**4 ore** (2 ore Parte A + 2 ore Parte B)

---

## Cost control - gestione dei costi

Valgono le stesse regole del modulo:
...
- **LAB11 completato** (Log Analytics + KQL funzionanti)
- **LAB12 completato** (almeno un alert creato)
- **ACI attiva** con log che arrivano in `ContainerInstanceLog_CL`
- **Workspace Log Analytics** raggiungibile dal portale
- file `azure_env.md` aggiornato

---

# ═══════════════════════════════════════════════════════════════
# PARTE A — Performance Analysis con KQL
# ═══════════════════════════════════════════════════════════════

# TEORIA — Perché analizzare le performance con KQL

# 1. Il limite dell'error_rate come unico indicatore

Nel LAB12 hai creato un alert basato sull'**error_rate**.

L'error_rate è un indicatore fondamentale, ma da solo non basta.

Un servizio può avere zero errori ed essere comunque inutilizzabile:

- risposte corrette ma lentissime
- un endpoint che impiega 5 secondi invece di 50 millisecondi
- degradi graduali che non producono errori ma peggiorano l'esperienza

Per questo motivo, nel mondo reale, le metriche di **latenza** e **distribuzione del traffico** sono altrettanto importanti quanto l'error rate.

---

# 2. Quali indicatori di performance analizzerai

In questa parte userai KQL per calcolare indicatori che non hai ancora esplorato:

- **Latenza media per endpoint** — quale endpoint è più lento?
- **Top endpoint per volume di traffico** — dove va il traffico?
- **Distribuzione degli status code** — quanti 200 vs 404 vs 500?
- **Trend della latenza nel tempo** — la latenza sta peggiorando?
- **Percentile 95 della latenza** — qual è il worst case realistico?

Questi indicatori ti danno una vista molto più completa rispetto al solo error_rate.

---

# 3. Perché il percentile è più utile della media

La media è un indicatore semplice ma ingannevole.

Esempio:

- 9 richieste da 10 ms
- 1 richiesta da 5000 ms
- media = 509 ms

La media dice 509 ms, ma il 90% degli utenti ha avuto 10 ms.
Il problema è solo su una richiesta.

Il **percentile 95** (p95) ti dice: il 95% delle richieste è stato completato entro X millisecondi.

È molto più utile per capire la realtà dell'esperienza utente.

---

# 4. Perché la distribuzione del traffico è importante

Sapere quante richieste riceve ogni endpoint è fondamentale perché:

- ti dice dove si concentra il carico
- ti aiuta a capire se un endpoint è sottodimensionato
- ti permette di prioritizzare gli interventi

Se il 90% del traffico va su `/time` e il degrado è proprio su `/time`, sai dove intervenire.

---

# 5. Perché analizzare i trend temporali

Un singolo valore di latenza media non ti dice se il sistema sta peggiorando.

Raggruppare la latenza in finestre temporali (per esempio 5 minuti) ti permette di vedere:

- la latenza sta crescendo?
- c'è stato un picco?
- il problema è continuo o episodico?

Questa è la base del ragionamento operativo Observability.

---

# 6. I dati che hai già

I tuoi log in `ContainerInstanceLog_CL` contengono già tutto il necessario.

Ogni riga ha un campo `Message` in formato JSON con:
...
Da questi campi puoi estrarre con `parse_json(Message)`:

- `.path` → endpoint
- `.status` → codice HTTP
- `.duration_ms` → latenza

Non serve configurare nulla di nuovo. Servono solo query KQL più avanzate.

---

# 7. Pipeline mentale della Parte A
...
# 8. Operatori KQL nuovi che userai

In questa parte userai operatori che nel LAB11 e LAB12 non hai ancora usato.

## `percentile()`

Calcola un valore percentile.

Esempio: `percentile(duration, 95)` restituisce il valore sotto cui cade il 95% dei dati.

## `extend`

Aggiunge una colonna calcolata a ogni riga.

Già visto nel LAB12 con `extend status = toint(parse_json(Message).status)`.

## `toreal()`

Converte un valore in numero decimale.

Utile quando `duration_ms` arriva come stringa dal JSON.

## `render`

Suggerisce al portale di visualizzare il risultato come grafico.

Esempio: `render timechart` produce un grafico temporale.
...
# ═══════════════════════════════════════════════════════════════
# PARTE B — Incident Simulation & Troubleshooting End-to-End
# ═══════════════════════════════════════════════════════════════

# TEORIA — Perché simulare un incidente

# 1. Il limite dell'analisi su dati "normali"

Nella Parte A hai analizzato dati provenienti da un'applicazione che funziona correttamente.

Gli endpoint rispondono velocemente, gli errori sono solo quelli provocati intenzionalmente con `/nope`.

In un sistema reale, però, i problemi non si presentano in modo così pulito.

Possono accadere:

- un endpoint che diventa improvvisamente lento
- latenze casuali che appaiono e scompaiono
- un aumento graduale dei tempi di risposta
- pattern difficili da vedere a occhio nudo

Per questo in questa parte modificherai l'applicazione per creare un **endpoint problematico**, lo distribuirai su Azure, genererai traffico e userai tutto ciò che hai imparato per investigare.

---

# 2. Che cos'è un incident nel contesto Observability

Un **incident** non è necessariamente un crash totale del sistema.

Spesso un incident è:

- un degrado di performance
- un aumento dell'error rate
- un servizio che risponde ma troppo lentamente
- una condizione anomala che impatta gli utenti

La capacità di **rilevare, investigare e documentare** un incident è una delle competenze più importanti nel mondo Observability.

---

# 3. Cosa farai in questa parte

In questa parte farai un esercizio realistico:

1. modificherai `app.py` aggiungendo un endpoint `/slow` con latenza casuale
2. ricostruirai l'immagine Docker (`obsapp:v5`)
3. ridistribuirai il container su ACI
4. genererai traffico misto (incluso `/slow`)
5. userai KQL per investigare il problema
6. creerai un alert sulla latenza
7. scriverai un mini incident report

---

# 4. Perché aggiungere un endpoint `/slow`

L'endpoint `/slow` simulerà un problema reale: un servizio che a volte risponde velocemente e a volte no.

Userai `random` per generare latenze casuali tra 10 ms e 3000 ms. Questo crea un pattern irregolare che rende l'investigazione più interessante rispetto a un errore fisso.

---

# 5. Che cos'è un incident report

Un **incident report** è un documento che descrive:

- cosa è successo
- quando è stato rilevato
- quali dati lo confermano
- quale è stata la causa
- quale azione è stata intrapresa

Nel mondo professionale, scrivere incident report è una pratica standard. In questo laboratorio ne scriverai una versione semplificata.

---

# 6. Pipeline mentale della Parte B

```text
modifica app.py → endpoint /slow
  ↓
docker build obsapp:v5
  ↓
push ACR + deploy ACI
  ↓
traffico misto
...
investigazione KQL
  ...
  ├── trend temporale
  ├── p95 per endpoint
  └── confronto /slow vs /health vs /time
  ↓
alert su latenza
  ↓
incident report
