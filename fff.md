...
```kql
ContainerInstanceLog_CL
| extend status = toint(parse_json(Message).status)   (*Usato Message al posto di LogEntry_s)
...
```kql
ContainerInstanceLog_CL
| extend path = tostring(parse_json(Message).path)   (*Usato Message al posto di LogEntry_s)
