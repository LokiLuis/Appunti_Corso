...
## 6.1 Dalla singola richiesta al comportamento del servizio

Nel LAB05 guardiamo le singole richieste.
Nel LAB06 guardiamo il sistema nel suo insieme.

Questa è l’idea chiave.

---

## 6.2 Che cos’è observability

Observability è la capacità di capire lo stato di un sistema osservandone gli effetti esterni.
...
Nel LAB06 lavoriamo soprattutto con:
...
## 6.3 Differenza tra log e metrica

### Log

È dettagliato, evento per evento.

Esempio:

```json
{"path":"/health","status":200,"duration_ms":3}
```

### Metrica

È un numero aggregato.
...
### Spiegazione da dare bene

Il log racconta **un episodio**.
La metrica racconta **un andamento**.

---

## 6.4 Che cos’è uno SLI

SLI significa **Service Level Indicator**.

È un indicatore quantitativo che descrive il comportamento del servizio.

Nel LAB06 useremo indicatori come:
...
Numero totale di richieste presenti nel log.

### Cosa indica

Il volume di traffico osservato.

### Esempio

Se ci sono 120 righe di log e ogni riga è una richiesta:
...
Percentuale di richieste che hanno dato errore.

### Formula

```text
error_rate = errori / richieste totali
```

### Esempio

100 richieste, 8 errori:
...
Va chiarito quali status consideriamo errore.
Di solito (nel nostro lab faremo così) ha senso considerare errore tutto ciò che è `>= 400`.
...
Tempo impiegato per servire una richiesta, in millisecondi.
...
Un servizio può essere corretto ma lento.
La latenza è un’informazione critica.

---

## 6.8 Percentili: p50 e p95



### p50

È il valore sotto cui cade il 50% delle richieste.
È molto vicino al concetto di mediana.

Se `p50 = 20ms`, metà delle richieste ha impiegato 20 ms o meno.

### p95

È il valore sotto cui cade il 95% delle richieste.

Se `p95 = 120ms`, il 95% delle richieste è stato completato in 120 ms o meno.

### Perché p95 è utile

Perché intercetta la coda delle richieste lente meglio della media.

Una media può nascondere problemi.
Il p95 li racconta molto meglio.
...
Classifica degli endpoint più richiesti.
...
## 6.10 `status_breakdown`

### Definizione

Distribuzione delle richieste per status code HTTP.
...
Permette di capire che tipo di errori sta producendo il sistema.

---

## 6.11 `sample_request_ids`

### Definizione

Un piccolo campione di request_id estratti dal log.

### Perché è utile

Permette di passare dal livello metrica al livello dettaglio.

Esempio:

1. prendi un request_id da `metrics.json`
2. fai grep nel log
3. ritrovi la richiesta precisa

Questa è una connessione importantissima tra dati aggregati e osservazione fine.

---

## 6.12 Che cos’è un parser

Un parser è un programma che legge un formato di input e ne estrae dati strutturati.

Nel LAB06 il parser:

* legge `logs/app.log`
* interpreta ogni riga JSON
* calcola gli indicatori
* scrive `metrics.json`

### Perché è importante

È il meccanismo che trasforma il log grezzo in informazione operativa.

---

## 6.13 Perché generare traffico artificiale

Nel LAB06 c’è `traffic.sh`.

### Scopo

Generare richieste verso il servizio per produrre un log abbastanza ricco da analizzare.

### Cosa fa

Invia chiamate ripetute a:

* `/health`
* `/time`
* `/echo`
* a volte endpoint sbagliati
* a volte JSON malformato
...
Quindi il parser può calcolare metriche più interessanti.

---

## 6.14 Interpretazione operativa dei numeri

Qui devi insistere molto.
I numeri non si raccolgono per sport.

### Esempio 1

`error_rate = 0.12`

Interpretazione: il 12% delle richieste fallisce.
È un valore alto. Va investigato.

### Esempio 2

`latency_p95_ms = 350`

Interpretazione: quasi tutte le richieste rientrano entro 350 ms, ma c’è una coda più lenta.

### Esempio 3

`top_paths` dominato da `/health`

Interpretazione: ci sono molte sonde di controllo o health check automatici.

---

# 7. Collegamento finale tra i tre lab

Devi chiuderla così, in modo che il gruppo capisca il filo logico:

## LAB04

Capisco se il problema è:
...
Creo un piccolo servizio HTTP locale con:
...
Prendo i log di quel servizio e li trasformo in:

* indicatori
* metriche operative
* prime valutazioni di observability
...
> Nei prossimi tre laboratori faremo un percorso completo.
> Prima impareremo a distinguere se un problema dipende dalla rete, dal DNS, da una porta o dall’applicazione.
> Poi costruiremo un piccolo servizio HTTP locale con endpoint semplici come `/health`, `/time` e `/echo`, e vedremo come ogni richiesta produce una traccia nel log.
> Infine trasformeremo quelle righe di log in indicatori operativi come error rate, p50 e p95, che sono la base del monitoring moderno e dell’observability.
...
Macchina collegata in rete.

## IP

Indirizzo che identifica l’host.

## Porta

Numero che identifica un servizio sull’host.

## DNS

Servizio che traduce nomi in IP.

## HTTP

Protocollo applicativo per comunicare con servizi web.
...
Processo che resta attivo e offre una funzione.

## Endpoint

Percorso HTTP, per esempio `/health`.
...
Identificatore univoco di una richiesta.

## Metrica

Valore aggregato che descrive il comportamento del sistema.

## SLI

Indicatore quantitativo di servizio.
...
Nel passo successivo posso trasformare tutto questo in una **scaletta parlata docente di 60-90 minuti**, proprio in stile:
**cosa dire, in che ordine, con mini demo terminale e punti da scrivere alla lavagna**.
